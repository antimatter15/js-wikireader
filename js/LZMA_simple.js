var LZMA = (function ()
{
    ...
    function decompress(byte_arr)
    {
        ...
        if (ioe) {
            console.log('Decompression failed: ' + ioe.detailMessage);
            return false;
        }
        try {
            ...
            if (instanceOf($e0, 11)) {
                e = $e0;
                console.log('Decompression failed: ' + e.detailMessage);
                return false;
            }
            else 
            throw $e0;
...
onmessage = function(e){
  var blockarr = e.data.split('').map(function(x){
    return x.charCodeAt(0)
  });
  postMessage(LZMA.decompress(blockarr));
}
